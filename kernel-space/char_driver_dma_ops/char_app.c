...
// Define ioctl command codes - must match driver definitions
#define MY_MAGIC 'S'  // 'S' for scaling
#define IOCTL_SCALE_IMAGE _IOW(MY_MAGIC, 1, struct scaling_params)

// Structure to pass scaling parameters to the driver
...
int main() {
    ...
    // Create scaling parameters struct
    struct scaling_params params = {
        .src_width = SRC_WIDTH,
    ...
    printf("[INFO] Starting %d iterations of image scaling using driver...\n", MAX_ITERATIONS);
    double start_time = omp_get_wtime();

    // Process multiple iterations using the driver instead of local scaling
    for (int i = 0; i < MAX_ITERATIONS; i++) {
        // Call the ioctl to perform scaling in the driver
        if (ioctl(fd, IOCTL_SCALE_IMAGE, &params) < 0) {
            fprintf(stderr, "[ERROR] ioctl IOCTL_SCALE_IMAGE failed: %s (errno: %d)\n", 
                    strerror(errno), errno);
            break;
        }
    }
    ...
    // Save the result as a PPM file (optional)
