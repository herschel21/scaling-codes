...
int main() {
    ...
    printf("[INFO] Starting DMA test application with kernel-based scaling\n");
    ...
    // Create scaling parameters for ioctl
    ...
    printf("[INFO] Starting %d iterations of kernel-based image scaling...\n", MAX_ITERATIONS);
    double start_time = omp_get_wtime();

    // Process multiple iterations using kernel scaling
    for (int i = 0; i < MAX_ITERATIONS; i++) {
        if (ioctl(fd, IOCTL_SCALE_IMAGE, &params) < 0) {
            fprintf(stderr, "[ERROR] Failed to perform kernel scaling: %s (errno: %d)\n", 
                    strerror(errno), errno);
            break;
        }
    }
    ...
    printf("[INFO] Completed %d kernel scaling operations in %.6f seconds (avg: %.6f sec/operation)\n", 
           MAX_ITERATIONS, total_time, avg_time);
           
    // Optionally save the output image to verify the result
    printf("[INFO] Saving output image for verification...\n");
    savePPM("output_kernel_scaled.ppm", &dstRes);
