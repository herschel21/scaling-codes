...
int init_gstreamer(const char* filename) {
    ...
    if (!appsink) {
        fprintf(stderr, "ERROR: Failed to get appsink\n");
    ...
    while (running) {
        ...
        if (!sample) {
            if (gst_app_sink_is_eos(GST_APP_SINK(appsink))) {
                ...
                fprintf(stderr, "ERROR: Failed to pull sample\n");
            }
            break;
        }
        ...
        if (!buffer) {
            fprintf(stderr, "ERROR: Sample has no buffer\n");
        ...
        if (!frame_buffer[frame_buffer_tail].data) {
        ...
        frame_buffer_count++;
        printf("DEBUG: Frame buffered - count: %d\n", frame_buffer_count);
...
int get_next_frame(uint8_t** frame_ptr) {
    pthread_mutex_lock(&buffer_mutex);
    while (frame_buffer_count == 0 && running) {
        printf("DEBUG: Buffer empty, waiting\n");
        pthread_cond_wait(&buffer_cond, &buffer_mutex);
    }
    if (frame_buffer_count == 0 && !running) {
        pthread_mutex_unlock(&buffer_mutex);
...
void init_video_texture() {
    ...
    printf("DEBUG: Max texture size supported: %d\n", max_texture_size);

    if (frame_width > max_texture_size || frame_height > max_texture_size) {
        fprintf(stderr, "ERROR: Video resolution %dx%d exceeds max texture size %d\n",
                frame_width, frame_height, max_texture_size);
        exit(EXIT_FAILURE);
    }
    ...
    if (err != GL_NO_ERROR) {
        fprintf(stderr, "ERROR: glTexImage2D failed with error %d\n", err);
        exit(EXIT_FAILURE);
    }
}
...
void render_loop() {
    ...
    while (running) {
        ...
        GLenum err = glGetError();
        if (err != GL_NO_ERROR) {
            fprintf(stderr, "ERROR: glTexSubImage2D failed with error %d\n", err);
            running = 0;
        }
        free(frame);
...
void cleanup_frame_buffer() {
    printf("DEBUG: Cleaning up frame buffer\n");
    pthread_mutex_lock(&buffer_mutex);
