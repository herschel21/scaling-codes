...
void prefill_frame_buffer() {
    ...
    while (frame_buffer_count < FRAME_BUFFER_SIZE) {
        unsigned char* frame;
...
// Unified frame getter (now just MP4)
int get_next_frame(unsigned char** frame_ptr) {
    printf("DEBUG: Getting next frame\n");
...
// Shader initialization (only RGBA now)
GLuint init_shaders() {
...
void render_fps(float fps) {
    char fps_str[6];
    snprintf(fps_str, 6, "%.1f", fps);
    int len = strlen(fps_str);

    float char_width = 20.0f / WINDOW_WIDTH;
    float char_height = 28.0f / WINDOW_HEIGHT;
    float start_x = -1.0f + 10.0f / WINDOW_WIDTH;
    float start_y = 1.0f - 10.0f / WINDOW_HEIGHT - char_height;
...
int main(int argc, char *argv[]) {
    ...
    printf("DEBUG: Using MP4 file source: %s\n", source_path);
    if (init_mp4_file(source_path) < 0) {
