...
#define FRAME_BUFFER_SIZE 50  // Increased from 10 to 50 for better buffering
...
int total_frames = 1800;        // Added: Total frames for the video (default for 30s at 60 FPS)
...
// Modified MP4 initialization with frame rate and total frames calculation
int init_mp4_file(const char* filename) {
    int ret;
    ...
    int64_t duration = format_context->duration;  // In AV_TIME_BASE units (microseconds)
    if (duration > 0) {
        double duration_seconds = (double)duration / AV_TIME_BASE;
        total_frames = (int)(duration_seconds / frame_duration);
        printf("Calculated total frames: %d (duration: %.3f s)\n", total_frames, duration_seconds);
    } else {
        total_frames = 1800;  // Fallback for 30s at 60 FPS
        printf("Could not determine duration, using default 1800 frames\n");
    }
...
int open_video_file(const char* filename) {
    ...
    char header[3];
    if (fread(header, 1, 3, video_file) != 3) {  // Check fread return value
...
int get_next_video_file_frame(unsigned char** frame_ptr) {
    if (fread(frame_data, 1, frame_width * frame_height * 3, video_file) != frame_width * frame_height * 3) {
        if (feof(video_file)) {
            ...
            char line[100];
            if (!fgets(line, sizeof(line), video_file)) {  // Check fgets return value
                fprintf(stderr, "Error reading header line after rewind\n");
                return -1;
            }
            int c = getc(video_file);
            while (c == '#') {
            ...
            ungetc(c, video_file);
            if (!fgets(line, sizeof(line), video_file)) {  // Check fgets return value
                fprintf(stderr, "Error reading width/height line after rewind\n");
                return -1;
            }
            if (!fgets(line, sizeof(line), video_file)) {  // Check fgets return value
                fprintf(stderr, "Error reading maxval line after rewind\n");
                return -1;
            }
            if (fread(frame_data, 1, frame_width * frame_height * 3, video_file) != frame_width * frame_height * 3) {
...
int init_egl() {
    ...
    if (!eglInitialize(egl_display, &major, &minor)) {  // Fixed: Proper eglInitialize call
...
// Modified render loop to fix premature ending and terminate after video
void render_loop() {
    unsigned char* frame;
    int frame_size;
    int frame_count = 0;
    struct timespec start_time, end_time, loop_start;
    ...
    while (running && frame_count < total_frames) {
        ...
        if (frame_size < 0) {
            fprintf(stderr, "Failed to get frame %d, attempting to continue\n", frame_count + 1);
            if (frame_count >= total_frames) break;  // Exit if we've reached the end
            usleep(frame_duration * 1000000);  // Wait and try next frame
            continue;  // Don’t exit, keep trying until total_frames
        }
        ...
        if (video_source_type == VIDEO_SOURCE_CAMERA) {
            ...
            if (frame_buffer_count < FRAME_BUFFER_SIZE) {
                ...
                if (next_size >= 0) {
                    ...
                    printf("Buffer refill failed at frame %d\n", frame_count);
                }
            }
        }
        ...
        total_time = (end_time.tv_sec - loop_start.tv_sec) + 
            (end_time.tv_nsec - loop_start.tv_nsec) / 1000000000.0;
        printf("Frame %d, total time: %.3f s\n", frame_count, total_time);
    ...
    printf("Render loop ended after %d frames, total time: %.3f s\n", frame_count, total_time);
    running = 0;  // Ensure program terminates after video ends
}
...
int main(int argc, char *argv[]) {
    ...
    if (strncmp(source_path, "/dev/video", 10) == 0) {
        video_source_type = VIDEO_SOURCE_CAMERA;  // Fixed: Full line restored
