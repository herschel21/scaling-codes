...
// MP4 frame handling (modified with flush)
int get_next_mp4_frame(unsigned char** frame_ptr) {
    int ret;
    ...
    while (!frame_finished) {
        ...
        if (ret < 0) {
            // Flush the decoder buffers before ending
            avcodec_flush_buffers(codec_context);
            printf("End of MP4 file reached\n");
            return -1; // Terminate instead of seeking
        }
...
// Frame buffering (modified to fix memory leak)
int get_buffered_frame(unsigned char** frame_ptr) {
    if (frame_buffer_count == 0) return -1;
    
    // Allocate new memory for the frame and copy data
...
// PPM frame handling (modified to terminate at end)
int get_next_video_file_frame(unsigned char** frame_ptr) {
    if (fread(frame_data, 1, frame_width * frame_height * 3, video_file) != frame_width * frame_height * 3) {
        if (feof(video_file)) {
            printf("End of PPM file reached\n");
            return -1; // Terminate instead of rewinding
        } else {
...
// Render loop (modified to terminate and free memory)
void render_loop() {
    unsigned char* frame;
    int frame_size;
    XEvent xev;
    ...
    while (running) {
        ...
        if (frame_size < 0) {
            if (video_source_type == VIDEO_SOURCE_CAMERA) {
                fprintf(stderr, "Failed to get next camera frame\n");
            } else {
                printf("Video playback completed\n");
                running = 0; // Terminate when video ends
            }
            break;
        }
        ...
        if (video_source_type == VIDEO_SOURCE_CAMERA) {
            ...
            free(frame); // Free the frame allocated by get_buffered_frame()
